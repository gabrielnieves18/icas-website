/**
 * Created by gaby on 6/25/17.
 *
 * LoginPage Messages
 *
 * This contains all the text for the LoginPage component.
 */
import { defineMessages } from 'react-intl';

export default defineMessages({
  accountTitleLabel: {
    id: 'spotin.components.EditorForms.accountTitle.label',
    defaultMessage: "Update your password",
  },
  accountDescriptionLabel: {
    id: 'spotin.components.Profile.accountDescription.label',
    defaultMessage: "Remember to make your password harder to guess by adding Numbers, Symbols, Capital Letters, and Lower-Case Letters and making it at least 12 characters long.",
  },
  accountPasswordDescriptionLabel: {
    id: 'spotin.components.Profile.accountPasswordDescription.label',
    defaultMessage: "Account Password",
  },
  authPrompLabel1: {
    id: 'spotin.components.EditorForms.auth.span.label1',
    defaultMessage: "Don't waste time with passwords.",
  },
  authPrompLabel2: {
    id: 'spotin.components.EditorForms.auth.span.label2',
    defaultMessage: 'Login with your facebook or google account.',
  },
  firstNameLabel: {
    id: 'spotin.components.EditorForms.firstName.label',
    defaultMessage: 'First Name',
  },
  fullNameLabel: {
    id: 'spotin.components.Profile.fullName.label',
    defaultMessage: "Full Name",
  },
  lastNameLabel: {
    id: 'spotin.components.EditorForms.lastName.label',
    defaultMessage: 'Last Name',
  },
  loginLabel: {
    id: 'spotin.components.EditorForms.login.label',
    defaultMessage: 'Login',
  },
  makeItDifficultLabel: {
    id: 'spotin.components.EditorForms.makeItDifficult.label',
    defaultMessage: 'Make it difficult to guess',
  },
  oneMoreTimeLabel: {
    id: 'spotin.components.EditorForms.oneMoreTime.label',
    defaultMessage: 'One more time',
  },
  registerLabel: {
    id: 'spotin.components.EditorForms.register.label',
    defaultMessage: 'Register',
  },
  retypeEmailLabel: {
    id: 'spotin.components.EditorForms.retype.email.label',
    defaultMessage: 'Retype email',
  },
  retypePasswordLabel: {
    id: 'spotin.components.EditorForms.retype.password.label',
    defaultMessage: 'Retype password',
  },
  submitLabel: {
    id: 'spotin.components.EditorForms.submit.label',
    defaultMessage: 'Submit',
  },
  emailLabel: {
    id: 'spotin.components.EditorForms.email.label',
    defaultMessage: "Email",
  },
  passwordLabel: {
    id: 'spotin.components.EditorForms.password.label',
    defaultMessage: 'Password',
  },
  phoneNumberLabel: {
    id: 'spotin.components.Profile.phoneNumber.label',
    defaultMessage: "Phone Number",
  },
  phoneNumberCommentLabel: {
    id: 'spotin.components.Profile.phoneNumberComment.label',
    defaultMessage: "Please only use numbers. Any other character is invalid.",
  },
  validationRequiredLabel: {
    id: 'spotin.components.EditorForms.validation.required.label',
    defaultMessage: 'Required',
  },
  validationInvalidEmailLabel: {
    id: 'spotin.components.EditorForms.validation.invalidEmail.label',
    defaultMessage: 'Invalid email address',
  },
  validationEmailMismatchLabel: {
    id: 'spotin.components.EditorForms.validation.mismatchEmail.label',
    defaultMessage: 'Email does not match',
  },
  validationPasswordMismatchLabel: {
    id: 'spotin.components.EditorForms.validation.mismatchPassword.label',
    defaultMessage: 'Password does not match',
  },
  validationPasswordWeakLabel: {
    id: 'spotin.components.EditorForms.validation.weakPassword.label',
    defaultMessage: "This is a weak password. Make it 12 character or longer to increase strength. Retype passwordOne more time.",
  },
});
